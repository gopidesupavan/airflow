...
from airflow.ti_deps.dependencies_states import EXECUTION_STATES, DEFERRED_STATES
    ...
    def _executable_task_instances_to_queued(self, max_tis: int, session: Session) -> list[TI]:
        ...
        for loop_count in itertools.count(start=1):
            ...
            for task_instance in task_instances_to_examine:
                ...
                msg_description_1 = ("Not executing %s since the number of tasks running or queued from DAG "
                                     "%s is >= to the DAG's max_active_tasks limit of %s")

                if task_instance.dag_model.max_active_tasks_include_deferred:
                    current_deferred_active_tasks_per_dag = concurrency_map_deferred.dag_active_tasks_map[
                        dag_id]
                    current_active_tasks_per_dag += current_deferred_active_tasks_per_dag

                    msg_description_0 = "DAG %s has %s/%s running, queued and deferred tasks"
                    msg_description_1 = ("Not executing %s since the number of tasks running or queued or "
                                         "deferred from DAG"
                                         "%s is >= to the DAG's max_active_tasks limit of %s"),
