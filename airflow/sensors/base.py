...
from airflow.models.taskinstance import run_trigger_task, TaskInstance
...
class InternalSensorTrigger(BaseTrigger):
    ...
    @provide_session
    async def get_context_from_ti(self, session: Session):
        dag_bag = DagBag()
        ...
        executable_task_instance, context = await run_trigger_task(ti)
        async with async_set_current_context(context):
            result = await executable_task_instance.async_sensor_wrapper(context=context)
    ...
    async def run(self) -> typing.AsyncIterator[TriggerEvent]:
        import cloudpickle
        try:
            poke_return = await self.get_context_from_ti()
...
class BaseSensorOperator(BaseOperator, SkipMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.run_in_triggers = run_in_triggers
    ...
    async def async_sensor_wrapper(self, context: Context):
        ...
        while True:
            try:
                if inspect.iscoroutinefunction(self.poke):
                    ...
                    self.log.info("inside while loop else")
    ...
    def execute(self, context: Context) -> Any:
        self.log.info("Intial context")
        self.log.info(context)
        self.log.info(self.task_id)
        self.log.info(context["ti"].map_index)
        if self.run_in_triggers:
            self.defer(
                trigger=InternalSensorTrigger(),
                method_name="execute_complete"
            )
        else:
            result = asyncio.run(self.async_sensor_wrapper(context=context))
            return result
