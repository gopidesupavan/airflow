...
class DataFusionEngine(LoggingMixin):
    ...
    def _register_object_store(self, datasource_config: DataSourceConfig, connection_config: ConnectionConfig):
        """Register object stores."""
        try:
            storage_provider = ObjectStorageProviderFactory.create_provider(datasource_config.storage_type)
            object_store = storage_provider.create_object_store(datasource_config.uri, connection_config=connection_config)
            ...
            raise ObjectStoreCreationException("Error while creating object store %s, %s", datasource_config.storage_type, e)
    ...
    def _register_data_source_format(self, datasource_config: DataSourceConfig):
        ...
        if datasource_config.table_name in self.registered_tables:
            raise ValueError(f"Table {datasource_config.table_name} already registered for {self.registered_tables[datasource_config.table_name]}, please choose different name")

        formate_cls = get_format_handler(datasource_config.format)
        formate_cls.register_data_source_format(self.session_context, datasource_config.table_name, datasource_config.uri)
        self.registered_tables[datasource_config.table_name] = datasource_config.uri
        self.log.info("Registered data source format %s for table: %s", datasource_config.format, datasource_config.table_name)
