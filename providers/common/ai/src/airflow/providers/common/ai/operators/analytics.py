...
class AnalyticsOperator(BaseOperator, CommonAIHookMixin):
    ...
    template_fields: Sequence[str] = ("datasource_configs","queries", "max_rows_check", "result_output_format")
    ...
    def execute(self, context: Context) -> Any:
        ...
        elif "json" in self.result_output_format:
    ...
    def _is_result_too_large(self, result_dict: dict[str, Any]) -> tuple[bool, int]:
        ...
        if max_rows_exceeded:
            self.log.warning("Query returned %s rows, exceeding max_rows_check (%s). Skipping result output as large datasets are unsuitable for return.", num_rows, self.max_rows_check)
    ...
    def _get_rows(result_dict: dict[str, Any], row_count: int) -> list[dict[str, Any]]:
        return [
            {key: result_dict[key][i] for key in result_dict}
