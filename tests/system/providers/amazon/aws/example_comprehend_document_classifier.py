...
ANNOTATION_KEY = "training-labels/label.csv"
...
# annotation file won't allow headers
...
DOCTOR_NOTES,doctors-notes.pdf-0,1
...
def copy_data_to_s3(bucket: str, sources: list[dict], prefix: str, number_of_copies=1):
    ...
    :param number_of_copies: Number of files to create for each element from the sources
    ...
    If number_of_copies is 2, file_name is 'file.pdf', and prefix is 'training-docs'.
    the resulting files will be:
        - training-docs/file-1.pdf
        - training-docs/file-2.pdf
    """
...
with DAG(
    ...
    training_docs = f"s3://{bucket_name}/{TRAINING_DATA_PREFIX}/"

    input_data_configurations = {
        "S3Uri": f"s3://{bucket_name}/{ANNOTATION_KEY}",
    ...
    chain(
        ...
        copy_data_to_s3(bucket=bucket_name, sources=PUBLIC_DATA_SOURCES, prefix=TRAINING_DATA_PREFIX,
                        number_of_copies=10),
        # TEST BODY
